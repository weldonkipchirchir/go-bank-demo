...
func TestPasetoMaker(t *testing.T) {
	...
	token, payload1, err := maker.CreateToken(username, role, duration)
	require.NoError(t, err)
	require.NotEmpty(t, token)
	require.NotEmpty(t, payload1, token)
...
func TestExpiredPasetoToken(t *testing.T) {
	...
	token, payload1, err := maker.CreateToken(util.RandomOwner(), util.DepositorRole, -time.Minute)
	require.NoError(t, err)
	require.NotEmpty(t, token)
